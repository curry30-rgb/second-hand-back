...
public class UserController {
    ...
    public ResultVo login(@RequestParam("accountNumber")
                              @NotEmpty @NotNull String accountNumber,
                          @RequestParam("userPassword")
                          @NotEmpty @NotNull String userPassword,
                          HttpServletResponse response) {
        String encode = MD5Utils.encode(userPassword);
        UserModel userModel = userService.userLogin(accountNumber, encode);
